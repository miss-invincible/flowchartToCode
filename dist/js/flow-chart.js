/**flow.chart.js - licensed under the MIT license. - Copyright (c) Bruno Roberto Burigo (brunorb8@hotmail.com) - https://github.com/BrunoRB/flow-chart.js - 2016-11-08*/
/**
 * Util Submodule
 */
...
var flow = (function(flow, doc) {
	...
	flow.Cache = {
		...
		setCache: function() {
			...
			//Cache.codePython = doc.getElementById('code_python');

			Cache.tabMenuList = Cache.diagramContainer.querySelector('.flow.tab.list');

			Cache.consoleArea = doc.getElementById('console-area');
			// Cache.editor2 = doc.getElementById('editor2');
			Cache.editor2 = document.querySelector('.editor2');
			Cache.fuck = document.querySelector('.fuck');
			...
			Cache.code = Cache.consoleArea.querySelector('.code_python');
			Cache.code = Cache.editor2.querySelector('.ace_editor');
		}
...
var flow = (function(flow, jsPlumb) {
	...
	var _examples = function() {
		var examples = JSON.stringify({"diagrams":{"Newdiagram-1-1-1-4-4-4-1-1-2-3-2-1-1-1-1-1-1-1":
...
var flow = (function(flow, doc, jsPlumb) {
	...
	StaticListeners._executionButtonsClick = function() {
		Util.on(Cache.consoleExecute, 'click', function(event) {
			...
			// flow.ExecutionHandler.generateCode();
			flow.UI.appendCodeToEditor();
			var data = code.join(" ");
			 console.log(data);
			code=[];
			// console.log("hello");
			...
			if (!(id in visiteds)) {
				...
				switch (shapeType) {
					...
					case flow.Const.SHAPE_TYPE.DISPLAY:
						var content = 'print ' + _getTextContent(shape);
						code.push("\n");
						for(var i=0;i<indent;i++)
						{
							code.push("	");
					...
					case flow.Const.SHAPE_TYPE.MANUAL_INPUT:
						var content = _getTextContent(shape).replace(
							/([a-zA-Z_$][0-9a-zA-Z_$]*)/, '$1 =int(raw_input("Insert the value of $1"))'
						);
						code.push("\n");
						for(var i=0;i<indent;i++)
						{
...
flow.UI = {
	...
	appendCodeToEditor: function(text){
		flow.ExecutionHandler.generateCode();
		var data = code.join(" ");
		console.log(data);
		code=[];

		// flow.Cache.editor2.insertAdjacentHTML('beforeEnd', "<code style='margin-left:100px'>"+data+"</code><br>");
		// flow.Cache.editor2.innerHTML = data;
		//flow.Cache.editor2.setValue("print hell");
		// $( ".editor2" ).append( "<p>Test</p>" );
		//flow.Cache.editor2.setAutoScrollEditorIntoView(true);
		...
		// flow.Cache.editor2.appendChild(newElement);
		flow.Cache.editor2.scrollTop = flow.Cache.consoleExhibition.scrollHeight;
	},
